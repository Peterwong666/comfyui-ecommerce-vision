...
### 1.1.1 🔒 G7 结果回填口径（**在拿到结果之前预先约定**）

> **为什么要"预先约定"**：本项目已在 G7 里犯过两次「先有机制推断、再拿它解释现象」的错误。
> 若等结果出来再决定怎么措辞，就会不自觉地**往已有结论上靠**（事后合理化）。
> 因此本节把**三种可能结果各自的措辞定死**，B 流拿到结果后**只做匹配、不做表述** —— 表述由本文件负责。
>
> **探针**：`engine/tools/probe_g7_weight.py`（B 流提供，三种模式 `klein-core` / `sdxl` / `klein-bnk`）。
> **探针只给一个客观信号**：同一 seed 下「带权重 vs 不带权重」的产物 **PNG IDAT 像素是否逐字节一致**。
> ⚠️ 探针**刻意不打印"生效/无效"**（那是语义判断，必须人工看图）—— 这个克制是对的，见下文情况 C。

| 情况 | 客观信号（像素） | 人工看图的判读 | **本文件的措辞（预先定死）** | 连带动作 |
|---|---|---|---|---|
| **A** | `klein-bnk` 的 baseline vs weighted **逐字节相同** | 无需判读 | 「**③ 已证伪**：把文本编码节点换成 `BNK_CLIPTextEncodeAdvanced` **也无法恢复权重** → "klein 支持权重"这条路**已试且无效，后人不必再试**」 | `weight_syntax_policy.flux2_klein_t2i_v1` **保持 FORBIDDEN**；白名单 `ADV_CLIP_emb` 条目注明「G7 ③ 已证伪，其"备选方案"定位**不再成立**」→ 转为可 disable |
| **B** | **不同** | **权重真的生效**（主体被增强/减弱、画面结构稳定，**无括号文字感**） | 「✅ **klein 可经 `BNK_CLIPTextEncodeAdvanced` 启用权重语法** —— 但**仅限使用该节点的工作流**；用核心 `CLIPTextEncode` 的链路**仍然严禁**」 | `weight_syntax_policy` 增加一条限定：「`klein + BNK_*` → allowed」；白名单 `ADV_CLIP_emb` → **keep 并接线**；B 流需出文件版本（`version+1`）并**重跑 L4**；本文件配方 `notes` 才可开始建议权重 |
| **C** | **不同** | **字面污染**（出现**括号/冒号/数字的文字感**、构图崩坏、或主体反而更难辨认） | 「**污染（非空转）** —— 且**换节点也救不回来**」+ 可操作建议：**移除括号与冒号，改用陈述句表述**（例：`(white mug:1.5)` → `a prominently featured white mug`） | `weight_syntax_policy.flux2_klein_t2i_v1` **保持 FORBIDDEN**，并在 `consequence` 里加这条「改用陈述句」的替代写法指导 |
| **D**（边界） | **不同** | **人工无法判定**属于 B 还是 C | 「**暂按污染处理**（保守取 C 的措辞）」，并显式标注「**人工无法判定**，非确证」 | 与 C 相同；另在 §5 待补齐项留一条复测 |

**四种情况共同的、不随结果变化的结论**（现在就成立，无需等实测）：
1. **klein 上默认一律严禁写权重** —— 因为我们默认用的是核心 `CLIPTextEncode`。
   情况 B 只是为**特定节点组合**开一个受限的口子，**不改变默认口径**。
2. 本提示词库**不含任何权重写法**，且已设为自检硬门禁（`WEIGHT_SYNTAX_IN_CONTENT` 必须为 NONE），
   因此**无论 A/B/C/D 哪种结果，都不需要回退任何已有内容**。
