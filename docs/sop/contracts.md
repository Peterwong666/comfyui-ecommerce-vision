...
| `retry_count` 上限来自配置 | `settings.task_max_retries = 3`，硬上限 5（`task_max_retries_hard_limit`）。**配置层已强制 `task_max_retries ≤ task_max_retries_hard_limit`**，启动时若违反会直接抛 `ValidationError` |
...
| 422 | 参数校验失败（EX-3） | 含边界值越界。**内容安全命中**（P6-13）也走 422：`detail` 为对象形式，`code=CONTENT_BLOCKED`（非 `ErrorType`，故用大写下划线）、`message` 只说明是哪一类问题、`fields` 为 `[{"key": "prompt", "reason": "…"}]`。⚠️ 不发明 451/403-内容码 —— 多一个表外的码就多一条前端分支，与上传校验因本表没有 413 而统一用 422 是同一个取舍。⚠️ `message` 与 `fields[].reason` **不回显命中的原词**（回显等于把词表泄露给攻击者，也等于把用户输入写回响应体）<br>**队列深度熔断**（EX-5，2026-09-17 实现）也走 422：`detail` 为**对象形式**、`code=queue_full`（§1.5 的 `ErrorType.QUEUE_FULL` 枚举值，故**小写**）、`message` 为「当前排队 N 个，预计等待 X 分钟」、`fields` 为 `[{"key": "queue_depth", "reason": "…"}]`。触发条件 = **在途任务数**（`queued` / `running` / `retrying`）**> 2 × `queue_max_depth`**（硬阈值）；被拒请求**不建任务、也不扣额**（检查排在扣额之前）。⚠️ 在途 > 软阈值（`queue_max_depth`，默认 10000）时**只告警不拒绝**（仍返回 202）—— `queue_full` 属**可重试**类型，此时拒绝用户是错的<br>**另有两条 422 用「非 `ErrorType` 的大写下划线码」**（2026-09-17 补登记，命名规则同本节开头）：<br>① **每用户在途任务数上限**（`settings.max_in_flight_per_user`）：`detail` 为**对象形式**、`code=TOO_MANY_IN_FLIGHT`（非 `ErrorType`，故**大写**）、`message` 给出「当前在途 N 个 / 本次需要 M 张 / 上限 L 张」、`fields` 为 `[{"key": "in_flight", "reason": "在途 N + 本次 M > 上限 L"}]`。判据用**本批的总张数**（不是"能塞几张算几张"，与「批量整体成功或整体失败」口径一致），**恰好等于上限放行**；该闸门只加在 `POST /tasks` 与 `POST /batches`，`retry` / `retry-failed` **不加**（重试不新增任务，加了等于同一个任务被计两次）。同被拒请求**不扣额、不建任务**（检查排在扣额之前）<br>② **工作流 Schema 里文本字段的长度上界**：`code=TEXT_LENGTH_OUT_OF_RANGE`（非 `ErrorType`，故**大写**）、`message` 为「{字段名} 长度不能超过 2000 个字符，当前 N」、`fields` 为 `[{"key": "<字段名>", "reason": "长度 N 超过上限 2000"}]`。按**工作流的 `param_schema`** 逐字段判（不硬编码 `prompt` / `negative_prompt` 键名），覆盖顶层与 `params` 两条通道。⚠️ **只查上界，不查下界** —— 契约 §3 的参数 Schema 没有 `required` / `allow_empty` 之类的标记，「哪个文本字段允许为空」**没有事实来源**，故空串对一般文本字段是合法输入（`negative_prompt: ""` 清空反向词即属此类）；顶层 `prompt` 的非空校验**不变**（`TaskSubmitIn` 的 `min_prompt_length = 1`）<br>③ **工作流 Schema 里整型字段的 min/max 边界**：`code=SIZE_OUT_OF_RANGE`（非 `ErrorType`，故**大写**）、`message` 为「{字段名} 需在 {min}-{max} 之间，当前 N」、`fields` 为 `[{"key": "<字段名>", "reason": "值 N 超出范围 [min, max]"}]`。按**工作流的 `param_schema`** 逐字段判（如 `i2i_v1` 的 `width` / `height` 为 512–2048），覆盖 `params` 通道。**闭区间**：边界值本身放行。<br>④ **工作流 Schema 里图片字段的 alpha 通道要求**：`code=REQUIRES_ALPHA`（非 `ErrorType`，故**大写**）、`message` 为「{字段名} 必须使用带透明通道（alpha）的 PNG」、`fields` 为 `[{"key": "<字段名>", "reason": "图片无 alpha 通道，局部重绘无法确定蒙版区"}]`。当前仅 `inpaint_v1.reference_image` 声明 `requires_alpha: true`；后端通过 `image_probe.has_alpha()` 只读 PNG 头部判定，无 Pillow 依赖。若素材不存在/越权/已删除 → **404**（与素材接口口径一致）；读存储失败 → **500** |
...
| 2026-09-17 | ① §2.2 的 **422 行补两个非 `ErrorType` 的码**：`TOO_MANY_IN_FLIGHT`（**每用户在途任务数上限**，用**本批总张数**判定、恰好等于上限放行、只加在 `POST /tasks` 与 `POST /batches`）与 `TEXT_LENGTH_OUT_OF_RANGE`（工作流 Schema **文本字段的长度上界**，按 `param_schema` 逐字段判）。② 同页明确：**文本字段的下界在 Schema 里无法表达**（无 `required` / `allow_empty` 标记），**空的反向词是合法输入**；顶层 `prompt` 的非空校验**不变** | ① 这两个码此前**已在实现里产生**（`backend/app/api/v1/tasks.py:91-92` 的模块常量）却**没进契约表** —— 前端只能对 422 盲判，而契约 §2.2 自己写着"需要前端分支处理时用对象形式的 detail"，码必须先登记。② 早先的实现对**所有** `type: text` 字段一并施加 `min_prompt_length`，使「清空反向词」（前端 `validate.ts` 的 `REQUIRED_TEXT_KEYS` 早已裁定为合法用法）在后端拿到 **422** —— 同一条产品规则出现两个相反的答案。按「Schema 没有 `required` / `allow_empty` 标记 ⇒ 下界**没有事实来源**」统一为**只查上界**（按字段名硬编码 `prompt` / `negative_prompt` 会造出第二份真相）。⚠️ **本节不改 §3 的 Schema 格式定义** —— 给 Schema 加显式标记属于契约变更，是**后续议题** | A（实现）· D（前端可在 422 上按 `detail.code` 分支，**无需改代码**：`errors.ts` 已能展示 `detail.message`）。⚠️ **§1 状态机、§3 参数 Schema、§4 埋点事件名、§2.3 接口清单（条数不变）均未动** |
| 2026-09-18 | ① 工作流 `verification` 枚举新增 **`render_path_l4_passed`**：`engine/registry.py` 的 `VALID_VERIFICATION` 扩展；`i2i_v1` / `inpaint_v1` / `upscale_v1` 的 `verification` 从 `pending_gpu` 迁移到 `render_path_l4_passed`，并补充 changelog。② `inpaint_v1.reference_image` 新增 `requires_alpha: true`。③ 后端任务提交新增两道校验：整型字段 min/max 越界 → 422 + `SIZE_OUT_OF_RANGE`；带 `requires_alpha` 的图片字段未通过 alpha 校验 → 422 + `REQUIRES_ALPHA`。④ `Settings` 新增 `model_validator` 强制 `task_max_retries ≤ task_max_retries_hard_limit`。⑤ §2.2 登记 `SIZE_OUT_OF_RANGE` / `REQUIRES_ALPHA`；§1.4 明确硬上限关系的配置层校验。 | ① `pending_gpu` 无法表达「L4 已真实出图但缺本口径 baseline 或核心语义未验证」的中间状态，导致 3 条工作流被迫挂在 `pending_gpu` 而 Registry -loader/校验器其实拿不到真实信息。② 防止 inpaint 无 alpha 时静默 no-op（输出与输入逐像素相同）。③ 把字段级边界与 alpha 要求落到后端，避免前端被绕过后造成不可重试的 GPU 浪费。④ 给 `task_max_retries_hard_limit` 一个真实的消费者，避免「硬上限只是注释」的契约缺口。 | A（实现 + 校验）· B（Registry 枚举）· D（表单需识别 `requires_alpha` 并在前端提示；无新状态码需要处理逻辑，只需展示文案）。⚠️ **§1 状态机、§4 埋点事件名、§2.3 接口清单（条数不变）均未动** |
