...
| EX-5 排队超限 | 「当前排队 N 个，预计等待 X 分钟」 | **两层**：在途 > 软阈值（`queue_max_depth`）→ 告警但**仍接单**（202）；在途 > 硬阈值（2 ×，默认 20000）→ **拒绝提交**（422 + `code=queue_full`），不建任务、不扣额。⚠️ 本节原写「保留在队列，不拒绝」，与 `PRD_v1.md` EX-5 的「拒绝提交」**冲突**，2026-09-17 已统一到与实现一致的两层语义 |
