...
| **`G-05`** | 离线补测与缺口决策 | ① 补 `AC-N5` 鉴权用例 · ② 补 `AC-F6` catalog 用例 · ③ **`params` 通道缺口**（提示词长度 / 生成分辨率）的实现取舍 · ④ `task_max_retries_hard_limit` 是否补消费者 · ⑤ `queue_max_depth` / 磁盘阈值是否实现 | 🟡 **进行中**（2026-09-17：① ② ✅ 已补；③ 提示词长度半边已裁定为「**只查上界**」并落地，**生成分辨率半边仍是缺口**；④ ⑤ **待决策**） | **无**（纯离线） | ① ② 有用例；③④⑤ 有**明确决策记录**（做或不做，都要写清理由） |
