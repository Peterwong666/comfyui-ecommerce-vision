...
| ID | 任务 | 产出 | Pri |
|---|---|---|---|
| P1-01 | 行业与市场分析：AIGC 电商视觉市场现状、规模、驱动因素、趋势 | 市场分析章节 | P1 |
| P1-02 | 竞品分析：Midjourney / SD WebUI / ComfyUI / 即梦 / 美图设计室 / 稿定 / Liblib / 吐司，按「一致性·可控性·批量·成本·门槛」五维打分 | 竞品对比表 + 结论 | P1 |
| P1-03 | 商业模式与定价假设（按张 / 订阅 / 私有化） | 商业模式章节 | P2 |
| P1-04 | **MRD 成稿**（背景·市场·用户·竞品·机会点·商业价值·成功指标） | `docs/mrd/MRD_v1.md` | P1 |

#### 1B. 用户与需求

| ID | 任务 | 产出 | Pri |
|---|---|---|---|
| P1-05 | 用户画像 ×4（运营 / 设计师 / 品牌方 / 开发者），含目标、痛点、场景、决策因素 | `docs/persona/persona.md` | P1 |
| P1-06 | 用户旅程地图（从「想做主图」到「图上架」全流程触点与情绪曲线） | 旅程图 | P1 |
| P1-07 | 需求池 + RICE 打分排序 | 需求池表格 | P1 |
| P1-08 | 用户访谈 ≥5 位真实电商从业者（或二手资料补足），验证痛点 | 访谈纪要 | P1 |

#### 1C. 产品需求（PRD）

| ID | 任务 | 产出 | Pri |
|---|---|---|---|
| P1-09 | 功能清单 + MoSCoW 分级（Must/Should/Could/Won't） | 功能清单表 | P0 |
| P1-10 | **非功能需求**：性能（P95 出图时长）、并发、可用性、安全、兼容性 | NFR 章节 | P0 |
| P1-11 | 任务状态机设计（pending→queued→running→success/failed/cancelled + 各异常分支） | 状态机图 | P0 |
| P1-12 | 异常与边界：OOM、超时、非法参数、磁盘满、并发超限、重复提交 | 异常处理表 | P0 |
| P1-13 | **PRD v1.0 成稿 + 自评审**（功能·交互·边界·数据·埋点·验收） | `docs/prd/PRD_v1.md` | P0 |

#### 1D. 流程与原型

| ID | 任务 | 产出 | Pri |
|---|---|---|---|
| P1-14 | 用户流程图（注册→上传→选模板→调参→生成→筛选→下载） | `docs/flow/user_flow.md` | P1 |
| P1-15 | 系统流程图 / 数据流图（前端→API→队列→ComfyUI→存储→回传） | `docs/flow/system_flow.md` | P1 |
| P1-16 | 线框图（低保真，覆盖 6 个核心页面） | 线框图 | P1 |
| P1-17 | 高保真原型 + 交互说明（可点，至少覆盖主流程） | `docs/prototype/` | P1 |
| P1-18 | **项目管理图**：WBS 分解 + 甘特图 + 里程碑 + 关键路径 | `docs/prd/gantt.md` | P1 |
| P1-19 | 资源安排与预算表（GPU 机时、存储、工具、人力） | `docs/prd/resource.md` | P1 |
| P1-20 | 埋点方案设计（事件表：事件名·属性·触发时机·用途） | `docs/prd/tracking_plan.md` | P1 |
