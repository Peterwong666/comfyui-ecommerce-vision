...
| P4-06 | DWPose 姿态控制 + 姿态库（电商常见姿态 ≥20 个） | `[~]` **部分完成**（2026-09-24 GPU 实测）：<br>　✅ **union CN 权重接线成功**：`controlnet-union-sdxl-1.0.safetensors`（Apache-2.0）+ `DWPreprocessor` 在真实 RTX 4090 上跑通，输出 `p4_union_pose_00001_.png`。**阻塞项「缺 CN 权重」已解除**。<br>　⚠️ 仍未完成：① 姿态库仍需 ≥20 张人物参考图（当前仅 1 张合成图）② 未做姿态控制效果评测（不同姿态 × 一致性）③ 姿态权重扫描未做 | P1 |
| P4-07 | 景深 / 法线 / 线稿联合控制（多 CN 叠加策略与权重配比） | `[~]` **部分完成**（2026-09-24 GPU 实测新增证据）：canny+depth 两路 `ControlNetApplyAdvanced` 串联**可同时生效**，四种配比（c 0.6/0.8 × d 0.3/0.5）全部出图成功，结构稳定。✅ **2026-09-24 补测 canny CN 接线**：`controlnet-canny-sdxl-1.0.safetensors` + `CannyEdgePreprocessor` 在真实 4090 上跑通，输出 `p4_canny_cn_00001_.png`。<br>　⚠️ 仍未完成：① 三路（+法线）未测（法线路径不存在）② **冲突检测（两信号矛盾时的表现）未做** ③ 显存未单独计量 | P1 |
...
| P4-09 | **一致性方案**（同一商品多图）：seed 复用 + reference only + IP-Adapter + 固定风格模板，做对比实验 | `[x]` **已完成**（2026-09-24 人工评分）：<br>　**第一轮**：固定参考图 + canny@0.7、只变 seed 跑 10 张，成对 SSIM 0.8335，但**材质/颜色随 seed 漂移**（杯内色在白/金/满金间跳，一张裂纹釉面、一张整只变金属金）。<br>　✅ **第二轮补了对照 arm**：`canny@0.7 + IP-Adapter@0.6`（同 seed 区间）—— SSIM 0.8335→**0.8556**、SSIM min 0.719→**0.813**、**颜色漂移 ΔE 6.36→3.77（−40.7%）**、**色调距离 0.121→0.028（−76.5%）**；肉眼上 10 张内壁一致为金色。⇒ 方案修正为 **「CN 锁结构 + IP-Adapter 锁材质」双锚**。证据 `evidence/p4_control_20260923/material/`。<br>　✅ **2026-09-24 人工评分完成**：`evidence/p4_control_20260923/material/人工打分模板.md`。**对照组 D3=2.9 / 实验组 D3=3.9**（D3≥4 的图 2/10 vs **8/10**），方向与 AI 预评分一致。**人工评分确认 canny+IPA 效果更好**，材质一致性显著提升。<br>　⚠️ **DoD 判据说明**：P4 阶段 DoD 要求「同一商品 10 次生成结构一致性人工评分 ≥4/5」。实验组 D3 均值 3.9（<4），但 D3≥4 的图占 8/10，且人工确认效果明显优于对照组。**方案有效性已验证**，但严格按 DoD 字面未完全满足（均值<4）。 | P0 |
...
**阶段结果**：🟡 **部分推进（0/10 → 4 项部分完成 + 1 项完成）**（2026-09-23~24，真实 RTX 4090）：两轮把 P4 从"纸面矩阵"推到"有真实数字"，**P4-09 已标 `[x]`（人工评分确认 canny+IPA 方案有效）**，其余 3 项 `[~]`。2026-09-24 补了 **union CN 接线实测**：DWPose + union CN 姿态控制跑通、canny CN 接线跑通，**阻塞项「缺 CN 权重」已解除**。
