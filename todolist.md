...
| P8-10 | 测试用例集：功能 / 边界 / 性能 / 兼容 / 安全（对应 PRD 验收） | `[~]` **定义完成 · 未实测**（`docs/review/test_plan.md` + `tests/README.md` + **29 条**边界用例）：PRD 共 **31 条 AC** ⇒ **覆盖 8 / 部分 12 / 🔴 无用例 11**（`8+12+11=31`；11 条清单已落表）。✅ **本轮补上两项此前零覆盖的 AC**：`AC-F6`（模板库，`backend/tests/test_catalog_templates.py` **10 条**）与 `AC-N5`（接口全部鉴权 / 端口暴露，`backend/tests/test_auth_coverage.py` **36 条** —— 用**遍历路由表**实现，新路由会被自动纳入；⚠️ 实现上踩到 FastAPI 的 `_IncludedRouter`，**必须用 `app.openapi()`**，见 `项目进展.md` #44）。✅ 文本字段长度校验（`params` 通道）的缺口已补（`code=TEXT_LENGTH_OUT_OF_RANGE`）；⚠️ **仍余 1 项是「钉住缺口」而非「守住期望」**：生成分辨率的 `params` 通道（`width`/`height` 越界时 API **不拒**，实测 511 / 2049 / 4096 均返回 202）。⚠️ 无覆盖率工具，**不给任何覆盖率数字**；**性能 / 兼容两层整体未执行** | P0 |
