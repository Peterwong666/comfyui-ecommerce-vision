...
| P6-08 | 配额与限流（按用户 / 按接口 / 并发上限） | ⚠️ **部分完成**（2026-09-17 凌晨）：**按用户配额**已修掉一个真实缺陷 —— 扣减原本是 Python 层读-改-写（`user.quota_used += N`），而 `get_db()` 每请求一个 session、同步路由跑在线程池里 ⇒ **并发提交会丢更新、额度被超发**。改为 `app/services/quota.py::charge()` 的**单条条件 UPDATE**（`WHERE quota_used + n <= quota_total`，`rowcount` 判成败，天然原子），4 个调用点（submit / batch / retry / retry-failed）全部改用它，`_assert_quota` 删除。402 的 `detail` 由纯字符串改为**对象形式**（`code` = `ErrorType.QUOTA_EXCEEDED` / `message` / `fields`），前端判据同步为「`detail.code` 优先、402 状态码后备」。补 `backend/tests/test_quota.py`（+8 条，此前**零覆盖**）。<br>　❌ **仍未做（故本行保持 `[ ]`）**：**限流**（按接口 / 并发上限 / `queue_max_depth`）、**重试是否二次扣额**、**失败是否退还额度** —— 本次**只修原子性与错误体，扣减口径与产品语义保持原样**，未超范围 | P0 |
...
- ❌ 仍未完成（**未计入完成数**）：P6-07（API Key 鉴权；JWT 已实现）、P6-08（**部分完成** —— 按用户配额的**原子性缺陷已修**（`app/services/quota.py` 条件 UPDATE）+ 402 结构化错误体 + 8 条配额测试；**限流 / 并发上限 / 重试二次扣额 / 失败退额仍未做**）、P6-09（部分完成，见上）、P6-11（多租户隔离）、P6-12（审计日志 —— `AuditLog` 模型与 auth/tasks/assets/worker 多处写入口已存在，但**覆盖范围与对外查询能力未验收**）、P6-13（内容安全）。
