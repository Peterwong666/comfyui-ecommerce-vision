...
**阶段结果**：🔵 进行中（**8/12**，2026-09-17）。已完成 P3-01/02/04/05/06/09/11/12 —— 三条新工作流（i2i/inpaint/upscale）的定义与 Schema 完成并通过 L1/L2/L3，**且渲染路径 L4 已于 2026-09-17 在真实 RTX 4090 上跑通**（三条均 **通过 11 · 失败 0 · 跳过 0**）；P3-03（WF-T2I）由 t2i_v1 承担且 L4 已通过；**P3-07 口径已按 ADR-008 裁定 1 修正：V1 明确不做、顺延 V2，故 V1 工作流口径为「5 条可用 + 1 条顺延」（原口径「6 条」不可能在 V1 达成）**。
...
| ID | 任务 | 产出 | Pri |
|---|---|---|---|
| P8-01 | **构建 golden set**：电商 30-50 个 case（品类 × 材质 × 光影 × 构图），含输入与期望 | `[~]` **定义完成 · L4 已跑通 42/42 · 期望值仍为 null** —— `tests/golden_set/cases/` **42 个用例**已由 `deploy/autodl/39_golden_set_run.py` 在 2026-09-18 真实出图（RTX 4090，v0.36.0，--highvram），产物在 `deploy/comfyui/evidence/golden_set_20260918/`。🔴 **`expected` 全为 `null`、`expected_status` 全为 `pending_gpu`**（这是刻意的：评分与 expected 回填必须由人工/评审完成，不许机器编造）。另建 `tests/golden_set/reference/.gitkeep`，把 `.gitignore` 那条**空转的例外规则落实**（见 `项目进展.md` #39） | P0 |
...
| P8-07 | A/B 对比机制（同 seed 下对比 模型 / 参数 / CN 权重） | `[ ]` **未开始** —— 无工具、无定义。同 seed 可比性已由 P8-09 覆盖 **5/5** | P1 |
...
| P8-08b | 回归测试（**真实 golden set 回归**）：变更后跑一遍真实评测 | `[~]` **真实出图 42/42 已完成 · 评分与 expected 回填待人工** —— 2026-09-18 已由 `39_golden_set_run.py` 真实出图，产物在 `deploy/comfyui/evidence/golden_set_20260918/`。但 `expected` 仍为 `null`，未做画质打分，因此不能算「完整回归通过」。后续工作流/模型变更后需重跑 | P0 |
...
| P8-10 | 测试用例集：功能 / 边界 / 性能 / 兼容 / 安全（对应 PRD 验收） | `[~]` **定义完成 · 分辨率缺口已补**（`docs/review/test_plan.md` + `tests/README.md` + **31 条**边界用例）：PRD 共 **31 条 AC** ⇒ **覆盖 9 / 部分 12 / 🔴 无用例 10**（`9+12+10=31`）。✅ 本轮补 `AC-F6`（模板库 10 条）、`AC-N5`（接口鉴权 36 条）、`SIZE_OUT_OF_RANGE`（`width`/`height` 按 Schema min/max 校验）、`REQUIRES_ALPHA`（inpaint 输入必须含 alpha）。⚠️ 无覆盖率工具，**不给任何覆盖率数字**；**性能 / 兼容两层整体未执行** | P0 |

**DoD**：良品率有数、有看板、可下钻；缺陷知识库 ≥30 条；模型/工作流变更有回归门禁。

**阶段结果**：🔵 进行中（**5/10**，2026-09-19 复核）。`[x]` **P8-02 / P8-04 / P8-06 / P8-08a / P8-09** 五项；`[~]` P8-01 / P8-08b（L4 已跑、评分未做）；`[ ]` P8-03a / P8-03b / P8-05 / P8-07 / P8-10。
