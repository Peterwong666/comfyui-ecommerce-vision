...
| 6 | **「无用例」是结论，不是待办占位符** | §2.4 列出的 13 条 AC **确实一条对应用例都没有**。不要因为"看起来该有"就当成漏登记 |
...
|---|---|---|---|
| 后端 | `backend/tests/` | `cd backend && .venv/bin/python -m pytest -q` | **469 passed** |
...
> ⚠️ 「469」这个数字**绑定 revision**：它含本批新增的 `backend/tests/test_boundary_values.py`
> （23 条）。去掉它，提交态基线是 **446**。数字脱离 revision 没有意义 —— 这是本项目
> 已经吃过一次亏的地方（`SUMMARY.md:175`）。
...
|---|---|---|---|---|
| AC-F1 | 6 条标准工作流全部上线且可跑通 | 🔴 **无用例** | — | 仓库只有 **5** 条有可执行 JSON，`style_transfer_v1` 注册了但无权重；「可跑通」需 GPU。**两个词都不成立** |
...
| AC-F6 | 模板库可用且按品类分组 | 🔴 **无用例** | 路由存在（`catalog.py` 的 `/templates`、`/templates/categories`） | 仓库里**没有** `test_catalog.py` / `test_templates.py` |
...
| AC-6.6 | 全部边界值（§6.2）有对应测试用例 | ⚠️ **部分** | `test_boundary_values.py`（本批，23 条）+ `test_assets.py` 上传侧 + `test_queue_guard.py`（队列深度，2026-09-17） | 见 §3：**队列深度已于 2026-09-17 由「无行为用例」补齐**；**2 项是「钉住缺口」而非「守住期望」** |
...
| AC-N5 | ComfyUI 端口不对外暴露，接口全部鉴权 | 🔴 **无用例** | 实现存在（`deps.py:32/41/48` 的 401；`config.py:166-176` 的 `0.0.0.0` 守卫） | **没有任何 401 用例**，也没有 `0.0.0.0` 守卫用例 —— 「全部鉴权」这个断言目前**只靠读代码** |
...
| 档 | 条数 | AC 列表 |
|---|---|---|
| ✅ **覆盖** | **6** | `AC-5.4` · `AC-6.2` · `AC-6.3` · `AC-F7` · `AC-F8` · `AC-P4`（结构层） |
| ⚠️ **部分** | **12** | `AC-5.1` · `AC-5.3` · `AC-5.5` · `AC-6.4` · `AC-6.5` · `AC-6.6` · `AC-F4` · `AC-F5` · `AC-N4` · `AC-N7` · `AC-P2` · `AC-P5` |
| 🔴 **无用例** | **13** | `AC-5.2` · `AC-6.1` · `AC-F1` · `AC-F2` · `AC-F3` · `AC-F6` · `AC-N1` · `AC-N2` · `AC-N3` · `AC-N5` · `AC-N6` · `AC-P1` · `AC-P3` |

> **6 + 12 + 13 = 31**（与 PRD 的 AC 总数对上）。
>
> ⚠️ 分档本身有主观成分（例如 `AC-F5` 的**元数据**半边是 ✅、**可复现**半边是 2/5，
> `AC-P4` 的**条数**有判据、**条目有效性**无判据）—— 本文件一律取**更保守**的那一档，
> 免得把半句话的达标读成整条达标。**但「无用例」的 13 条是确定的、无争议的。**
>
> 其中 `AC-F1` / `AC-F6` / `AC-N5` 是**纯离线也不该缺**的（`AC-N5` / `AC-F6` 连 GPU 都不需要），
> 属本索引暴露出的**真实覆盖缺口**，已登记在 §8。

#### 13 条「无用例」全列（**可直接当补测清单**）

| # | AC | 为什么现在没有 | 解除条件 |
|---|---|---|---|
| 1 | `AC-5.2` | 「≤1 采样步」是 ComfyUI 内部时序 | GPU + 真实出图 |
| 2 | `AC-6.1` | 「降级后**能成功**」要真跑 OOM 场景 | GPU |
| 3 | `AC-F1` | 只有 5 条工作流有 JSON；`style_transfer_v1` 缺权重 | 权重入库 + GPU |
| 4 | `AC-F2` | 人工 10 分钟实测 | GPU + 真人 |
| 5 | `AC-F3` | 用例数 <30 且通过率从未评测 | 补例 + GPU 评测 |
| 6 | `AC-F6` | 无人写过 catalog 接口测试 | **纯离线可补（不需要 GPU）** |
| 7 | `AC-N1` | 8h 压测 | GPU + 8h |
| 8 | `AC-N2` | 需真实批量统计成功率 | GPU |
| 9 | `AC-N3` | 需真正跑完 1000 张 | GPU |
| 10 | `AC-N5` | 无人写过鉴权 / 端口暴露用例 | **纯离线可补（不需要 GPU）** |
| 11 | `AC-N6` | `deploy/**` 从未执行 | 目标机 + 凭据 |
| 12 | `AC-P1` | 内测未开始 | 人工 |
| 13 | `AC-P3` | 看板未实现 | 先做 P8-05 |
...
| 4 | 提示词长度 **1 – 2000** | ❌ 无 | ➕ 顶层 `1`/`2000`（闭区间）· `2001`（拒）· `0`（拒）；**`params` 通道 2 条缺口用例** | **新增（含缺口）** |
...
| 10 | 队列深度：软阈值 **10000** / 硬阈值 **20000**（= 2×） | ✅ 行为：`test_queue_guard.py`（2026-09-17 新增 **11 条**） | ➕ 配置值钉死 + 软层「告警但**仍接单**」/ 硬层 **422 `queue_full`** / 被拒不扣额不建任务 / 恰好等于硬阈值放行 / 在途状态口径 / 失败与取消不退额 | ✅ **已有**（2026-09-17 实现后补齐） |

**结论（如实）**：
- **10 项里原先唯一无行为用例的「队列深度」已于 2026-09-17 补齐**（实现 + `backend/tests/test_queue_guard.py`，11 条）—— 此前不是漏写，是**实现不存在**；
- **2 项（提示词长度 / 生成分辨率的 `params` 通道）是「钉住缺口」**，不是守住期望（见 §4）；
- 其余 7 项有真实判据。

---

## 4. ⚠️ 两处「钉住缺口」的用例（**不是期望，是现状**）

本批新增的用例里有 4 条**刻意断言「当前是错的」**，用来把缺口变成可执行判据。
**修好之后这些用例会变红 —— 那正是它们的作用**：修的人必须同时把断言改成期望值。

| 用例 | 钉住的缺口 | 证据 | 修好后要改成 |
|---|---|---|---|
| `test_params_prompt_channel_ignores_max_length` | 走 `params["prompt"]` 时**突破 `max_prompt_length=2000`**：`TaskSubmitIn.params` 是裸 `dict[str, Any]`（`schemas/task.py:30`），而 `_validate_params`（`api/v1/tasks.py:86-103`）**只查 steps/cfg** | 实测 `2001` 字符返回 **202**，且 `Task.params["prompt"]` 长度确实是 **2001**（**真落库**，不是"被别处拦了只是码不同"） | 断言改 `422` |
| `test_params_prompt_channel_ignores_min_length` | `params["prompt"] = ""` 同样放行（`_merge_params` 的 `if v is not None` 会保留 `""`） | 实测返回 **202** | 断言改 `422` |
| `test_generation_side_out_of_bounds_is_not_rejected_by_the_api` | 生成分辨率 `width/height` 越界时 **API 不拒**：`min_gen_side`/`max_gen_side` 的唯一读取点是 `engine/validate.py` 的 `SETTINGS_BOUNDS_MAPPING`（当**工作流 Schema 的一致性地板**），**不拿用户传入值去比** | 实测 `511` / `2049` / `4096` 均返回 **202**；前端 `validate.ts:26-33` 亲口确认这个空洞 | 断言改 `422` |
| `test_retry_hard_limit_is_a_ceiling_over_the_default`（docstring 标注） | `task_max_retries_hard_limit` **没有任何代码读取** —— 全仓库只有 3 处：定义（`config.py:113`）、契约一句声明、本用例。`mark_retrying` 只看 `task_max_retries` | `grep -rn task_max_retries_hard_limit` 结果即证据 | 补消费者后去掉 docstring 的缺口标注 |

> **为什么不在这里顺手修实现**：这 5 个文件是**文档与用例**的交付范围，实现改动
> 会越过本任务的边界（且 `params` 通道的校验策略牵涉前端契约 §3.4，应由实现方定）。
...
| 文件 | 变异验证状态 | 出处 |
|---|---|---|
| `backend/tests/test_boundary_values.py` | ✅ 本批对 **2 条**新用例做了「故意改坏 → 变红 → 还原」 | 本任务交付报告 |
...
| 2 | **真实 PostgreSQL** —— 469 条后端测试**全部**跑在 SQLite 上 | 本机无 PG 二进制也无 docker daemon |
...
| # | 缺口 | 影响 | 解除条件 |
|---|---|---|---|
| 1 | **13 条 AC 无用例**（§2.6 全列） | 其中 **`AC-F6`（模板库）与 `AC-N5`（鉴权）连 GPU 都不需要就能补**，属**可立即消除**的缺口 | 见 §2.6 的「解除条件」列 |
| 2 | **`AC-N5` 鉴权零用例**（无 401 / 无端口暴露判据） | 「接口全部鉴权」目前**只靠读代码** | 纯离线补测 |
| 3 | **`AC-F6` 模板库零用例** | 路由存在但无人守 | 纯离线补测 |
...
```bash
# ① 后端全量（含本索引涉及的绝大多数用例）
cd backend && timeout 900 .venv/bin/python -m pytest -q          # → 469 passed

# ② 边界值 10 项专项（本批新增落点）
cd backend && timeout 600 .venv/bin/python -m pytest -q tests/test_boundary_values.py   # → 23 passed
...
grep -rn "test_catalog\|test_templates" backend/tests/     # AC-F6 → 应无命中
grep -rn "401" backend/tests/                              # AC-N5 → 应无命中
grep -rn "disk_min_free_gb\|disk_warn_free_gb" backend/    # AC-6.5 → 只有定义与注释
```

> ⚠️ 上面 ① 的 **469** 与 ② 的 **23** 绑定本次 revision。数字会随用例增减而变 ——
