...
| AC-6.6 | 全部边界值（§6.2）有对应测试用例 | ⚠️ **部分** | `test_boundary_values.py`（本批，23 条）+ `test_assets.py` 上传侧 | 见 §3：10 项里 **1 项仍无行为用例**、**2 项是「钉住缺口」而非「守住期望」** |
...
| 10 | 队列深度 **≤ 10000** | ❌ **无** | ➕ **仅配置值钉死**；`queue_max_depth` 全仓库**无读取点**（限流未实现）→ **刻意不写「看起来在测队列深度」的假用例** | 🔴 **仍无行为用例** |

**结论（如实）**：
- **10 项里 1 项（队列深度）仍无行为用例** —— 因为**实现不存在**，不是漏写；
...
| 5 | **`AC-6.6` 队列深度无行为用例** | `queue_max_depth` 无读取点（限流未做） | 先实现再补测 |
